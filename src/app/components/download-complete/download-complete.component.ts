...
@Component({
  ...
  template: `
    <div class="text-center flex flex-col items-center justify-center p-4">
      ...
      @if (isBatchDownload()) {
        <h2 class="text-3xl font-bold text-white mb-2">{{ 'COMPLETE_BATCH_TITLE' | translate }}</h2>
        <p class="text-zinc-400 mb-8" [innerHTML]="'COMPLETE_BATCH_SUBTITLE' | translate:{count: totalBatches()}"></p>
        <div class="h-28"></div> <!-- Spacer to align the "Go Again" button -->
      } @else {
...
export class DownloadCompleteComponent {
  ...
  isBatchDownload = input<boolean>(false);
  totalBatches = input<number>(0);
